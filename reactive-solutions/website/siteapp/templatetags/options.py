...
def get_services_description():
    return SiteOption.objects.filter(site_option_name='services_description').values_list('site_option_value', flat=True)[0]

@register.simple_tag
def get_projects_description():
    return SiteOption.objects.filter(site_option_name='projects_description').values_list('site_option_value', flat=True)[0]

@register.simple_tag
def get_blog_description():
    return SiteOption.objects.filter(site_option_name='blog_description').values_list('site_option_value', flat=True)[0]
